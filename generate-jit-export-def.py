...
def _parse_symbols(dumpbin, lib_path):
    ...
    if result.returncode != 0:
        print(f"Warning: dumpbin failed on {lib_path}: {result.stderr}", file=sys.stderr)
        return set(), set()
...
def main():
    if len(sys.argv) < 4:
        print(f"Usage: {sys.argv[0]} <dumpbin> <output.def> <lib1> [lib2 ...] [-- <backing1> ...]",
              file=sys.stderr)
        sys.exit(1)
    ...
    for lib in primary_files:
        ...
        print(f"  {lib}: {len(defined)} defined, {len(undefined)} undefined", file=sys.stderr)
    ...
    if backing_files and unresolved:
        ...
        print(f"  Backing libs resolved {backing_exported} additional symbols", file=sys.stderr)
    ...
    if len(all_symbols) > 65000:
        print(f"WARNING: {len(all_symbols)} symbols may exceed COFF limit of 65535",
              file=sys.stderr)
